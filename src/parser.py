...
def draw_line_search_system(main_par):
    ...
    google_str = f"https://google.com/search?q={main_par.get('query')}&hl={main_par.get('lan')}&tbs={date_search.get('google')}&start="
    yandex_str = f"https://yandex.ru/search/search/?text={main_par.get('query')}&lang={main_par.get('lan')}&within={date_search.get('yandex')}&p="
...
def research_url_sites(list_url_sites):
    ...
    for elem in list_url_sites:
        if not PurePosixPath(elem).suffix in (".pdf" or ".doc" or ".docx" or ".xlsx" or ".pptx" or ".ppt" or ".zip"):
